...
collector's allocation routine.
The transformation inspects all the graphs to discover the
\texttt{struct} types in use by the program, and assigns a unique type id to
each of them.  These type ids are then collected in internal tables that
...
kind of expressiveness increased our ability of reuse code even in
low-level helpers.
...
Current work also includes the OOTyper and back-ends for CLI/.NET and
Smalltalk/Squeak, all in-progress.
