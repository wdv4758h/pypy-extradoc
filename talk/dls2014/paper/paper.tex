...
\arigo{wrong, it is used.  maybe describe a bit more why}
...
\subsection{Memory Requirements\label{sub:memoryreq}}
...
size\footnote{Configurable value taken from experience with our
non-STM reference interpreter}. Additionally, the area used for the
read markers is lazily mapped by the kernel and only needed for
objects in the old object space. Thus, it can be at most the total
size of memory used by old objects divided by 16, adding up to
$6.3\%$ to the total memory requirements.
...
(incl. the sharing segment). This is certainly the worst case and
so far not observed in practice. During major collections we re-share
privatised pages that are currently not modified by any transaction.
...
except those in Section~\ref{sec:performance-bench}.
\cfbolz{the details about why the JIT confuses everything should go into
Section~\ref{sec:performance-bench}. here just write "because the integration
with the JIT is incomplete"}
We would like to
regard it as a simple performance enhancement, but that is not what
happens in reality. First, since the JIT~\cite{cfbolz09} bases its decision what to
compile on runtime profiling, running in multiple threads, it may compile
...
\cfbolz{can you explain how you measure the errors?}
...
\remi{reference \ref{sub:memoryreq}}
...
% old object space including the memory required for private pages. Note,
% it does not measure the memory required for the segment-private areas
% like the nursery and read markers. The
...
\remi{do this and RSS numbers for pypy-stm-nojit and normal pypy-nojit:}
...
\cfbolz{you can't kill it, otherwise a reviewer will ask. also, the pypy-no-stm numbers are meaningless as long as the options are different}
...
thread by optimistically running transactions in parallel.


% To isolate factors we look at performance w/o JIT and perf w JIT.
% w/o JIT:
%  - it scales
%  - slower than reference CPython
%  - gil scales negatively
% w JIT:
%  - it sometimes still scales (more work needed)
...
First we run our benchmarks on four
...
however, it does not consistently beat CPython, the reference implementation of
...
\subsection{JIT Benchmarks}
The speedups from enabling the JIT in
...
\bibitem{wayforward14} Remigius Meier, Armin Rigo. 2014. A Way Forward
  in Parallelising Dynamic Languages. ICOOOLPS'14.
