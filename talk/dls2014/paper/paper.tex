...
lock.  While this setup limits the benefits due to parallelism %% not
clear we must say so TRG % -- parallelism is limited to I/O or other
...
\item We introduce a new software transactional memory (STM) system
  that performs well for Python virtual machines.
...
%% can be dropped, not the core topic
...
%% What is really needed from this section?? Maybe the subsection on Python can be moved
%%  to the intro -- the rest is questionable.  TRG
...
% review of paper
We set out to resolve the issue of parallelism in Python by replacing
the GIL with TM. In particular we deemed a close integration of STM
and the GC best as a way to attack the issues of atomicity,
synchronisation, and memory management in a unified way. While the
interplay of STM and the GC gives us some important performance
benefits, we also implemented atomic blocks backed by STM for Python
as a scalable synchronisation mechanism.

% STM needs good performance:
Since in our approach, STM becomes a vital part of the interpreter,
good performance is essential. We found an interesting way to combine
the existing memory segmentation and virtual memory features of
current CPUs to lower the overhead of STM. We want to stress the
importance of memory segmentation to our approach, because it seems
rather unvalued. In the move from the x86 to the x86{-}64
architecture, they already removed 4 out of 6 segment registers.  We
want to make the point that there are still valid and interesting uses
for them (see also \cite{bennet10}).
...
still reached \remi{$1.9\times$}.

For the overall performance compared to our baseline interpreter
CPython, integration with our JIT compiler is necessary. Once this
integration matures, we believe our approach to not only serve as a
simple GIL replacement, but also as a way forward in parallelising
languages -- making atomicity and synchronisation the language's
instead of the programmer's problem.
