...
the execution, they can not make use of the increasing parallelism in
current hardware.
...
the garbage collector to provide atomicity, synchronisation, and
...
uses a combination of features present in current CPUs to lower its
overhead so that it can beat the performance of a GIL supported Python
interpreter already on 2 threads.
...
threads or cores. It is also used to implement atomic blocks for the
