...
A common trait of many of these language implementations is the use of
...
language interpreters. The key idea is a close integration of STM with
the garbage collector to approach atomicity, synchronisation, and
memory management in a unified way.  As a user abstraction for a
scalable synchronisation mechanism, we implement atomic blocks by
exposing part of the STM system to applications.  The described STM
uses a combination of features present in current CPUs, so that the
parallelisation benefits outweigh the system's overhead already on 2
threads when comparing to a GIL supported Python interpreter.
...
it requires writing in a different language.}. Because the GIL is only
seen as an implementation detail of the interpreter, applications are
...
location. We on the other side follow the direct approach of using TM
...
transactions -- providing the application with the guarantee of
atomicity for the enclosed instructions.
...
transaction inevitable, possibly aborting the current one if not
possible.
...
the object moved or was seen as garbage. In this simplified Application
...
place in a single transaction. Another issue is the support of
irreversible operations, like calls to external functions or general
input~/ output. If we are allowed to, we simply commit the transaction
before and start a new one after these operations. Within an atomic
block, however, we turn the transaction inevitable so that the
transaction cannot abort and strong isolation is guaranteed.
...
Note, these areas are at offsets inside the segments known at
compile time. This makes some checks very efficient, e.g.\ checking
if an object resides in the nursery only requires comparing its $SO$
to the boundaries of the nursery.
...
from traced objects. The STM system needs to record objects in the
write set, acquire write locks, and privatise their pages
(copy-on-write). It only needs to do this for old objects from
previous transactions, as the others are not known to other
transactions anyway.
...
flag again on all objects, including new overflow objects. The flag
...
iterations to get a single measurement.
...
during commit needs to check in all concurrently running transactions.
...
the others ($avg=2.0\times$ speedup). The reason for that is
...
GIL replacement yields an average speedup of $2.6\times$ for
