...
What we described so far has no mesurable performance impact by
itself, and it is missing conflict detection to be considered a
full-fledged STM system.  This requires adding \emph{barriers} to the
program to register the objects into the read or write set before
reading or writing to them\footnote{Barriers added by an automatic and
local program transformation~\cite{felber07}}.  Furthermore, we need
to describe the commit protocol.
...
memory used by the old objects.  With four segments, this can add
as much as $25\%$ to the total memory requirements.
...
Besides memory usage, we also want to mention scalability. We
think that current desktop machines with up to 8 cores should
be seen as the main area of applications of dynamic languages.
This is why our evaluation was performed on an off-the-shelf
desktop CPU with only 4 cores. In the future, we will look
at how far this STM system can really scale.
