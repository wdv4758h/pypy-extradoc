...
\cfbolz{this needs a better section title. "Transactional Memory Implementation"?}
...
\subsubsection{Memory Segmentation and Page Sharing}
...
\subsubsection{Memory Segmentation on the x86}
...
in the local segment, then the objects' binary content would differ
...
\item [{Write~Barrier:}] This is triggered by checking a flag in the
  ...
  collector (not described here) which requires a similar write barrier
...
(there is typically a very high turn-around rate in dynamic languages).
...
compiler\footnote{Clang 3.5 with some patches to fix bugs in its address-space
