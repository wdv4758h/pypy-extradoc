...
threads read and write data inside their own segment; exchanges
between segments occur only when transactions commit.  So far, this is a
model that would also work for distributed transactional memory.
...
\item [{Read~Barrier:}] A major contribution to our performance results
  ...
  approach allows the read barrier to be a single, unconditional memory
  write into segment-local memory.
 We managed to
